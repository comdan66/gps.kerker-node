...
const Progress = {
  ...
  percent(text) {
    ...
    if (Progress.color)
      lines[0] += (Progress.count ? Progress.color.gray('(' + Progress.index + '/' + Progress.count + ')').dim() + ' ' + Progress.color.black('─', true).dim() + ' ' + percent() : '') + (text ? ' ' + Progress.color.black('─', true).dim() + ' ' + text : '')
    else
