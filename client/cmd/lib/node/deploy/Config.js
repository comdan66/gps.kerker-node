...
module.exports = (app, closure) => {
  ...
  queue.enqueue(next => Progress.block('檢查設定檔是否存在', Progress.cmd('執行動作', 'check ' + Path.relative(App.path('root'), App.path('cfg-main')) + ' is exists'))
    .doing(progress => FileSystem.existsSync(App.path('cfg-main'))
      ? progress.success('存在')
      : progress.failure(null, '尚未設定 ' + Color.gray('Serve', true) + ' 設定檔！'))
    .go(next))
  ...
  queue.enqueue(next => App.config.argvs['--goal'] != 's3'
    ? Progress.block('檢查 GitHub 參數', Progress.cmd('執行動作', 'check GitHub argvs'))
      ...
      .doing(progress => {
        ...
        App.config.deploy.github.account === null ? progress.failure(null, '部署至 GitHub 需給予正確的 ' + Color.gray('--account', true) + ' 參數') : progress.counter
        App.config.deploy.github.repository === null ? progress.failure(null, '部署至 GitHub 需給予正確的 ' + Color.gray('--repository', true) + ' 參數') : progress.counter
    ...
    : Progress.block('檢查 S3 參數與功能', Progress.cmd('執行動作', 'check S3 argvs、features'))
      ...
      .doing(progress => {
        ...
        bucket === null ? progress.failure(null, '部署至 S3 需給予正確的 ' + Color.gray('--bucket', true) + ' 參數') : progress.counter
        access === null ? progress.failure(null, '部署至 S3 需給予正確的 ' + Color.gray('--access', true) + ' 參數') : progress.counter
        secret === null ? progress.failure(null, '部署至 S3 需給予正確的 ' + Color.gray('--secret', true) + ' 參數') : progress.counter
        ...
        App.config.deploy.s3.instance.listBuckets((error, data) => !error
          ? data.Buckets.map(t => t.Name).indexOf(App.config.deploy.s3.bucket) == -1
            ? progress.failure(null, '您這組 ' + Color.gray('access') + '、' + Color.gray('secret') + ' 無法操作 ' + Color.gray(Config.bucket, true) + ' 此 Bucket！')
            : progress.success()
  ...
  queue.enqueue(next => Progress.block('檢查 PHP 主要檔案是否存在', Progress.cmd('執行動作', 'check ' + Path.relative(App.path('root'), App.path('lib-php-main')) + ' is exists'))
    .doing(progress => FileSystem.existsSync(App.path('lib-php-main'))
      ? progress.success('存在')
      : progress.failure(null, 'PHP 主要檔案 ' + Color.gray(Path.relative(App.path('root'), App.path('lib-php-main')), true) + ' 不存在！'))
    .go(next))
