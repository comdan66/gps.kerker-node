...
const scanPort = (min, max, closure) => {
  ...
  Progress.doing('檢查 Server port ' + Color.gray(min, true), Progress.cmd('執行動作', 'listening ' + min))
  Progress.total(1)
...
const resError = (params, message) => {
  ...
  response.end()
  showLog && Progress.doing('接收 ' + Color.gray(method + '：' + pathname, true) + ' 請求', Progress.cmd('Request', method + ' ' + pathname)).total(1).success('回應：' + message)
}
...
const resDocu = params => {
  ...
  if (extension == 'php') {
    ...
    return Exec(cmds.join(' '), { maxBuffer: App.config.enablePHP.maxBuffer }, (error, stdout) => {
      ...
      for (let i = 0, l = Math.ceil(cmds.length / 2); i < l; i++)
        i ? details.push(' '.repeat(4) + Color.gray(cmds[i * 2], true).dim() + ' ' + Color.gray(cmds[(i * 2) + 1], true).dim().italic())
          : details.push(Color.gray(cmds[i * 2], true).dim() + ' ' + Color.gray(cmds[i * 2 + 1], true).dim().italic())

      response.writeHead(error, { 'Content-Type': 'text/html; charset=UTF-8' })
      response.write(addReloadAction(stdout))
      response.end()
      
      showLog && Progress.doing('接收 ' + Color.gray(method + '：' + pathname, true) + ' 請求', Progress.cmd('Request', method + ' ' + pathname), ...details).total(1).success('回應：' + error)
    })
  }
  ...
  FileSystem.readFile(file, { encoding: extension && App.config.serve.server.utf8Exts.indexOf('.' + extension) != -1 ? 'utf8' : null}, (error, data) => {
    ...
    response.end()
    showLog && Progress.doing('接收 ' + Color.gray(method + '：' + pathname, true) + ' 請求', Progress.cmd('Request', method + ' ' + pathname)).total(1).success('回應：200')
  })
}
  ...
  try {
    ...
    App.socketIO.sockets.on('connection', socket => {
      ...
      socket.on('disconnect', _ => {
        ...
        showLog && Progress.doing('失去 Socket 連線', Progress.cmd('Socket', 'disconnect'), Color.gray('目前共有 ', true).dim() + Color.gray(sockets.size, true) + Color.gray(' 個連線', true).dim()) && Progress.total(1) && Progress.success()
      })
      showLog && Progress.doing('新增 Socket 連線', Progress.cmd('Socket', 'connection'), Color.gray('目前共有 ', true).dim() + Color.gray(sockets.size, true) + Color.gray(' 個連線', true).dim()) && Progress.total(1) && Progress.success()
    })
    Progress.success(), closure()
  } catch (e) {
    return Progress.failure('失敗'),
      process.stdout.write("\r" + ' '.repeat(5) + Color.purple('↳').dim() + ' 錯誤原因：' + Color.gray(e.message, true) + "\n"),
      process.stdout.write("\r" + ' '.repeat(5) + Color.purple('↳').dim() + ' 無法使用 ' + Color.red('LiveReload') + " 功能\n"),
      process.stdout.write("\r" + ' '.repeat(5) + Color.purple('↳').dim() + ' 其他功能 ' + Color.green('ICON') + Color.gray('、', true).dim() + Color.green('SCSS') + " 皆能正常使用\n"),
      closure()
  }
}

const openHttpsServer = closure => {
  Progress.doing('開啟 ' + Color.cyan('https', true) + ' 伺服器', Progress.cmd('執行動作', 'run https Server, port:' + App.config.serve.server.port)), Progress.total(1)
  try {
    const https = require('https').Server(App.config.serve.server.ssl)
    Progress.do()
    https.on('error', e => Progress.doing(Color.cyan('https', true) + ' 發生錯誤', Progress.cmd('錯誤原因', e.message)) && Progress.failure())
...
const openHttpServer = closure => {
  Progress.doing('開啟 ' + Color.purple('http', true) + ' 伺服器', Progress.cmd('執行動作', 'run http Server, port:' + App.config.serve.server.port)), Progress.total(1)
  try {
    const http = require('http').Server()
    Progress.do()
    http.on('error', e => Progress.doing(Color.purple('http', true) + ' 發生錯誤', Progress.cmd('錯誤原因', e.message)) && Progress.failure())
...
module.exports = (app, closure) => {
  ...
  if (!isNaN(App.config.serve.server.port))
    return Progress.doing('檢查 Server port ' + Color.gray(App.config.serve.server.port, true), Progress.cmd('執行動作', 'listening ' + App.config.serve.server.port)), Progress.total(1), portStatus(App.config.serve.server.port, (free, error) => Progress.doing() && free
      ? Progress.success() && server(App.config.serve.server.port)
      : Progress.failure('已被使用', 'port ' + Color.gray(App.config.serve.server.port, true) + ' 已被使用中！'))

  Progress.doing('檢查 Server port ' + Color.gray(App.config.serve.server.port.default, true), Progress.cmd('執行動作', 'listening ' + App.config.serve.server.port.default)), Progress.total(1)
